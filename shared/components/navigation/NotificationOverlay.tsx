...
export function NotificationOverlay() {
  ...
  const getNotificationIcon = (type: string) => {
    switch (type) {
      ...
      default: return <Bell className="w-5 h-5 text-gray-600 dark:text-slate-300 flex-shrink-0" />;
    }
  };
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'read': return <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 dark:bg-slate-700 text-gray-800 dark:text-slate-200">Прочитан</span>;
  ...
  return (
    <>
      ...
      <div data-notification-overlay className="fixed left-0 right-0 top-16 mx-auto w-[720px] bg-white dark:bg-slate-800 dark:bg-slate-800 rounded-xl shadow-2xl border border-gray-200 dark:border-slate-700 dark:border-slate-700 z-[100001] overflow-hidden" style={{ pointerEvents: 'auto' }}>
        
        {/* Filters */}
        {showFilters && (
          <div className="absolute left-0 top-full w-full bg-white dark:bg-slate-800 dark:bg-slate-800 rounded-b-xl shadow-xl border border-gray-200 dark:border-slate-700 dark:border-slate-700 z-10 p-4">
                ...
                {['all:Все','unread:Непрочитанные','read:Прочитанные','archived:Архив'].map(v => {
                  ...
                  return (
                    <button key={val} onClick={() => setFilters({ status: val as any })}
                      className={`w-full text-left px-3 py-2 rounded text-sm ${filters.status === val ? 'bg-blue-50 dark:bg-slate-700 text-blue-700 dark:text-blue-400 font-medium' : 'text-gray-700 dark:text-slate-300 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-700 dark:bg-slate-700 dark:hover:bg-slate-700'}`}>
                      {label}
                    </button>
                  );
                })}
                ...
                {['all:Все','overdue:Просроченные','low-stock:Склад (мало)','out-of-stock:Склад (нет)','finance:Финансы','schedule:График','system:Система'].map(v => {
                  ...
                  return (
                    <button key={val} onClick={() => setFilters({ category: val as any })}
                      className={`w-full text-left px-3 py-2 rounded text-sm ${filters.category === val ? 'bg-blue-50 dark:bg-slate-700 text-blue-700 dark:text-blue-400 font-medium' : 'text-gray-700 dark:text-slate-300 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-700 dark:bg-slate-700 dark:hover:bg-slate-700'}`}>
                      {label}
                    </button>
                  );
                })}
                ...
                {['all:Все','high:Высокий','medium:Средний','low:Низкий'].map(v => {
                  ...
                  return (
                    <button key={val} onClick={() => setFilters({ priority: val as any })}
                      className={`w-full text-left px-3 py-2 rounded text-sm ${filters.priority === val ? 'bg-blue-50 dark:bg-slate-700 text-blue-700 dark:text-blue-400 font-medium' : 'text-gray-700 dark:text-slate-300 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-700 dark:bg-slate-700 dark:hover:bg-slate-700'}`}>
                      {label}
                    </button>
                  );
                })}
              </div>
              {hasActiveFilters && <button onClick={clearFilters} className="w-full text-center text-sm text-blue-600 dark:text-blue-400 font-medium">Сбросить фильтры</button>}
            </div>
          </div>
        )}

        {/* Header */}
        <div className="p-4 border-b border-gray-200 dark:border-slate-700 dark:border-slate-700 bg-white dark:bg-slate-800 dark:bg-slate-800">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-semibold text-gray-900 dark:text-white dark:text-white">Уведомления</h3>
        ...
        {filtered.length === 0 ? (
          <div className="p-6 text-center">
            ...
            <p className="text-gray-500 dark:text-slate-400 dark:text-slate-400 text-sm">Нет уведомлений</p>
            {hasActiveFilters && <button onClick={clearFilters} className="mt-2 text-sm text-blue-600 dark:text-blue-400">Сбросить фильтры</button>}
          </div>
        ) : (
          <div className="divide-y divide-gray-100 dark:divide-slate-700 dark:divide-slate-700 max-h-[500px] overflow-y-auto">
            {filtered.map(n => (
              <div key={n.id} onClick={() => n.status === 'unread' && markAsRead(n.id)}
                className={`block border-l-4 ${getPriorityColor(n.priority)} hover:bg-gray-50 dark:hover:bg-slate-700 dark:bg-slate-700 dark:hover:bg-slate-700 bg-white dark:bg-slate-800 dark:bg-slate-800 ${n.status === 'unread' ? 'bg-blue-50 dark:bg-slate-750' : ''} ${n.status === 'archived' ? 'grayscale' : ''}`}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-xs text-gray-500 dark:text-slate-400">{n.priority === 'high' ? 'Высокий' : 'Средний'}</span>
                      </div>
                      <p className={`text-sm ${n.status === 'unread' ? 'font-medium' : ''}`}>{n.message}</p>
                      <p className="text-xs text-gray-500 dark:text-slate-400 mt-1">{new Date(n.date).toLocaleDateString('ru-RU')}</p>
