...
interface OverheadTableProps {
  ...
  onRowChange: (index: number, field: string, value: string | number | null) => void;
...
export function OverheadTable({
  ...
  const total = editIndex !== null
    ? editItems.reduce((sum, item) => sum + (item.cost || 0), 0)
                ...
                return (
                  <tr key={key} className={`border-b border-gray-100 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700 dark:bg-slate-700 ${editIndex === i ? 'bg-blue-50 ring-2 ring-blue-300' : ''}`}>
                    ...
                    <td className="py-3 px-4">
                      <input
                        ref={(el) => registerInput(`${key}-name`, el)}
                        value={item.name || ''}
                    ...
                    <td className="py-3 px-4">
                      <input
                        ...
                        value={item.category || ''}
                        onChange={(e) => onRowChange(i, 'category', e.target.value || null)}
