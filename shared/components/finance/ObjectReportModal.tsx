...
export function ObjectReportModal({ isOpen, onClose, onSubmit }: ObjectReportModalProps) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Отчет по объекту">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1.5">Название объекта</label>
            <input
              type="text"
              value={objectName}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1.5">Номер договора</label>
            <input
              type="text"
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1.5">Тип операции</label>
            ...
            {([
              ...
              <button
                ...
                className={`flex-1 rounded-lg border px-4 py-2 text-sm font-medium transition-all ${
                  type === t.value ? `${t.color} ring-2 ring-offset-1` : 'bg-white dark:bg-slate-800 text-gray-600 dark:text-slate-300 border-gray-200 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700 dark:bg-slate-700'
                }`}
              >
                {t.label}
              </button>
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full rounded-lg border border-gray-300 dark:border-slate-600 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 resize-none"
            rows={3}
