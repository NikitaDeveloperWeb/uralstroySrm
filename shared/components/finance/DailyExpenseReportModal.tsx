...
export function DailyExpenseReportModal({ isOpen, onClose, onSubmit }: DailyExpenseReportModalProps) {
  ...
  const [items, setItems] = useState<ExpenseItem[]>([
    { id: generateId(), category: 'fuel', amount: '', description: '' },
  ]);
  ...
  const addItem = () => {
    setItems([...items, { id: generateId(), category: 'other', amount: '', description: '' }]);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setNote('');
    setItems([{ id: generateId(), category: 'fuel', amount: '', description: '' }]);
    onClose();
  };

  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Ежедневные расходы">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">Позиции расходов</label>
            ...
            {items.map((item) => (
              <div key={item.id} className="flex gap-2 items-start">
                <select
                  value={item.category}
                  onChange={(e) => {
                    const newItems = items.map(i => i.id === item.id ? { ...i, category: e.target.value } : i);
                    setItems(newItems);
                  }}
                  className="rounded-lg border border-gray-300 dark:border-slate-600 px-3 py-2 text-sm focus:border-orange-500 focus:outline-none focus:ring-1 focus:ring-orange-500 min-w-[140px]"
                >
                  {categories.map((cat) => (
                ...
                <input
                  ...
                  value={item.amount}
                  onChange={(e) => {
                    const newItems = items.map(i => i.id === item.id ? { ...i, amount: e.target.value } : i);
                    setItems(newItems);
                  }}
                  className="flex-1 rounded-lg border border-gray-300 dark:border-slate-600 px-3 py-2 text-sm focus:border-orange-500 focus:outline-none focus:ring-1 focus:ring-orange-500"
                  min="0"
                  step="0.01"
                  required
                />
                <input
                  ...
                  className="flex-[2] rounded-lg border border-gray-300 dark:border-slate-600 px-3 py-2 text-sm focus:border-orange-500 focus:outline-none focus:ring-1 focus:ring-orange-500"
                />
                {items.length > 1 && (
                  <button
                    type="button"
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1.5">Примечание</label>
          <textarea
            value={note}
            onChange={(e) => setNote(e.target.value)}
            className="w-full rounded-lg border border-gray-300 dark:border-slate-600 px-3 py-2 text-sm focus:border-orange-500 focus:outline-none focus:ring-1 focus:ring-orange-500 resize-none"
            rows={2}
