...
export function WeatherWidget({ city = 'Екатеринбург' }: WeatherWidgetProps) {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-lg shadow-md p-4">
      <h3 className="text-sm font-semibold text-gray-700 dark:text-slate-300 mb-3">Погода в {city}</h3>
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2">
            <span className="text-4xl font-bold text-gray-900 dark:text-white">{weather.temp}°C</span>
            <span className="text-gray-500 dark:text-slate-400">{weather.condition}</span>
          </div>
